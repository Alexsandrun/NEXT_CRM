...
def test_bootstrap_login_whoami_and_logout():
    ...
    login_payload = {"tenant": "demo", "email": "admin@demo.local", "password": "admin123"}
    r2 = requests.post(f"{BASE}/api/auth/login", json=login_payload, timeout=5)
    assert r2.status_code == 200, r2.text
    tok = r2.json().get("access_token")
    assert tok, r2.text

    r3 = requests.get(f"{BASE}/api/auth/whoami", headers={"Authorization": f"Bearer {tok}"}, timeout=3)
    assert r3.status_code == 200, r3.text

    r4 = requests.post(f"{BASE}/api/auth/logout", headers={"Authorization": f"Bearer {tok}"}, timeout=3)
    assert r4.status_code in (200, 204), r4.text

    r5 = requests.get(f"{BASE}/api/auth/whoami", headers={"Authorization": f"Bearer {tok}"}, timeout=3)
